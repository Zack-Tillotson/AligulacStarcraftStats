...
$result = mysql_query("select e.name, date_format(m.date, '%b %d, %Y') date, p1.id ida, p1.tag taga, p1.race rca, lower(p1.country) flaga, m.sca, p2.id idb, p2.tag tagb, m.scb, p2.race rcb, lower(p2.country) flagb from ratings_event e, ratings_match m, ratings_player p1, ratings_player p2 where eventobj_id = e.id and p1.id = m.pla_id and p2.id = m.plb_id and e.fullname like 'WCS 2013 Season 2 America Premier Ro16 %' order by e.name asc, m.id asc") or die(mysql_error());
$rows = array();
while($row = mysql_fetch_array( $result )) {
  if($row['name'] == "Group A" && count($rows) == 0) { # Violet forfeit vs Oz
  ...
  if($row['name'] == "Group A" && count($rows) == 3) { # Violet forfeit vs Ryung
    array_push($rows, array(
      'name' => 'Group A',
      'date' => 'Aug 05, 2013',
      'ida' => '8',
